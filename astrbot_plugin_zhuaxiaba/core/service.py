...
class ZhuaXiaBaService:
    ...
    @staticmethod
    def _extract_author_name(item: dict[str, Any]) -> str:
        """从帖子/楼层数据中提取作者名，支持 author 对象和直接字段。"""
        # 1. 尝试从 author 对象中获取
        ...
        if isinstance(author_obj, dict):
            name = (
                ...
                or author_obj.get("user_nickname")
                or author_obj.get("name")
            )
            if name:
                return str(name)
        
        # 2. 尝试直接从 item 中获取常见字段
        name = (
            item.get("author_name")
            or item.get("user_name")
            or item.get("nickname")
            or item.get("user_nickname")
            or item.get("name_show")
            or item.get("username")
        )
        if name:
            return str(name)
        
        # 3. 如果 author 是字符串，直接使用
    ...
    async def search_threads(self, keyword: str, limit: int = 3) -> list[dict[str, Any]]:
        """
        搜索相关话题
        """
        threads = await self.list_threads(sort_type=0)
        results = []
        for thread in threads:
            if keyword in thread.get("title", "") or keyword in thread.get("content", ""):
    ...
    async def get_thread_detail(self, *, thread_id: Any, pn: int = 1, r: int = 0) -> dict[str, Any]:
        ...
        main_post = {
            ...
            "author": self._extract_author_name(first_floor),
    ...
    async def list_replyme(self, *, pn: int = 1) -> list[dict[str, Any]]:
        ...
        for idx, item in enumerate(raw_items[:20], start=1):
            ...
            username = self._extract_author_name(item)
            result.append(
                {
                    "index": idx,
                    "thread_id": thread_id,
