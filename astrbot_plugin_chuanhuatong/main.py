...
# ==============================================================================
# 【公共 API 区域】供其他插件（如 WorldBook）调用
# ==============================================================================

async def switch_preset_public_api(context, target_name: str, session_id: str) -> tuple[bool, str, str]:
    """
    通用的预设切换接口
    ...
    参数:
        ...
        target_name: 目标预设名称
        session_id: 会话唯一标识 (event.unified_msg_origin 或 event.get_session_id())
    
    返回:
        (success: bool, message: str, preset_name: str)
    """
    try:
        # 1. 获取当前插件实例
        # 注意：这里的 name 必须与 @star.register(name="...") 中的名字完全一致
        # 请检查你的 main.py 头部，假设是 "astrbot_plugin_chuanhuatong" 或 "chuanhuatong"
        PLUGIN_NAME = "astrbot_plugin_chuanhuatong" # <--- 请根据实际情况修改此字符串
        plugin_inst = context.get_star_by_name(PLUGIN_NAME)
        
        if not plugin_inst:
            logger.error(f"[PresetAPI] 未找到插件实例：{PLUGIN_NAME}")
            return False, "❌ 内部错误：未找到传话筒插件实例。", ""

        # 2. 调用插件内部的核心切换方法 _switch_preset
        # 这是最关键的一步：直接复用原有的私有逻辑
        if not hasattr(plugin_inst, '_switch_preset'):
            logger.error("[PresetAPI] 插件实例缺少 _switch_preset 方法")
            return False, "❌ 内部错误：插件逻辑缺失。", ""

        # 执行切换
        success, message, preset_name = plugin_inst._switch_preset(target_name, session_id)
        
        if success:
            logger.info(f"[PresetAPI] 外部调用切换预设成功：{preset_name} (会话：{session_id})")
        else:
            logger.warning(f"[PresetAPI] 外部调用切换预设失败：{message}")
            
        return success, message, preset_name

    except Exception as e:
        logger.error(f"[PresetAPI] 切换预设时发生异常：{e}", exc_info=True)
        return False, f"❌ 切换出错：{str(e)}", ""
# ==============================================================================
# 【公共 API】供其他插件直接调用
# ==============================================================================

# 全局插件实例引用
_chuanhuatong_instance = None

def get_chuanhuatong_instance():
    """
    获取传话筒插件的唯一实例。
    供其他插件（如 WorldBook）调用。
    返回: ChuanHuaTongPlugin 实例 或 None
    """
    return _chuanhuatong_instance
# ==============================================================================
# 【原有代码继续...】
# ==============================================================================
@dataclass
...
class ChuanHuaTongPlugin(Star):
    ...
    def __init__(self, context: Context, config: Optional[AstrBotConfig] = None):
        ...
        # 【新增】将当前实例注册到全局变量
        global _chuanhuatong_instance
        _chuanhuatong_instance = self
        logger.info("[传话筒] 实例已注册到全局变量。") # <--- 可选：加个日志确认
        ...
        self._keep_text_sessions_file = self._data_dir / 'keep_text_sessions.json'
        self._keep_text_sessions = self._load_keep_text_sessions()
    ...
    def _load_keep_text_sessions(self) -> set[str]:
        if not self._keep_text_sessions_file.exists(): return set()
        try:
            data = json.loads(self._keep_text_sessions_file.read_text(encoding='utf-8'))
            return set(data) if isinstance(data, list) else set()
        except Exception: return set()

    def _save_keep_text_sessions(self):
        try: self._keep_text_sessions_file.write_text(json.dumps(list(self._keep_text_sessions)), encoding='utf-8')
    ...
    def _pick_user_role_character(self, role: str, emotion_key: str) -> str:
        ...
        if role == self.ROLE_LEGACY:
            if preferred_folder:
                ...
                if legacy_dir.exists():
                    path = self._pick_random_asset(legacy_dir, {".png", ".webp", ".jpg"})
                    if path:
                        self._last_character_path = path
                        return path
            path = self._pick_random_asset(self._user_char_dir, {".png", ".webp", ".jpg"})
            if path:
                self._last_character_path = path
            return path or ""
        ...
        for directory in candidate_dirs:
            if directory.exists():
                path = self._pick_random_asset(directory, {".png", ".webp", ".jpg"})
                if path:
                    self._last_character_path = path
                    return path
        path = self._pick_random_asset(role_dir, {".png", ".webp", ".jpg"})
        if path:
            self._last_character_path = path
        return path or ""
    ...
    def _pick_builtin_character(self, emotion_key: str) -> str:
        ...
        path = self._pick_random_asset(target_dir, {".png", ".webp", ".jpg"})
        self._last_character_path = path or ""
        return path or ""
    ...
    async def _render_split_text(
        ...
        """
        分割长文本并渲染为一张或多张图片，返回 Comp.Image 组件列表。
        不再直接发送消息，由调用方决定如何处理结果。
        """
        ...
        for chunk_text, chunk_emotion in chunks_with_emotion:
            path = await self._render_with_fallback(chunk_text, chunk_emotion, session_id)
            if path:
        ...
        if enable_merge and len(rendered_paths) > 1:
            max_per_batch = max(1, int(self.cfg().get("merge_max_images", 5) or 5))
            batches = [
                rendered_paths[i : i + max_per_batch]
                for i in range(0, len(rendered_paths), max_per_batch)
            ]
        
            final_paths = []
            for batch in batches:
                if len(batch) == 1:
                    final_paths.append(batch[0])
                else:
                    merged_path = await asyncio.to_thread(self._merge_images_vertical, batch)
                    if merged_path:
                        # 原图短时清理
                        for p in batch:
                            self._schedule_cleanup(p, delay=5.0)
                        final_paths.append(merged_path)
                    else:
                        final_paths.extend(batch)  # 合并失败，保留原图
            rendered_paths = final_paths

        # 转为组件列表
        components = []
        for p in rendered_paths:
            self._schedule_cleanup(p, delay=90.0)
            components.append(Comp.Image.fromFileSystem(p))
        return components
    ...
    async def inject_emotion_prompt(self, event: AstrMessageEvent, req: ProviderRequest = None):
        if not self._is_session_enabled(event): return
        # 如果 req 为 None，尝试从 event 获取
        if req is None:
            req = getattr(event, 'provider_request', None)
        if req is None:
            return  # 没有请求，直接返回
    ...
    @filter.on_llm_response(priority=10)  # 提供优先级，确保在其他插件之前优先清洗
    async def get_emotion_tag(self, event: AstrMessageEvent, resp: LLMResponse = None):
        if not self._is_session_enabled(event): return
        if resp is None:
            resp = getattr(event, 'llm_response', None)
        if resp is None:
            return  # 没有响应，直接返回
    ...
    async def on_decorating_result(self, event: AstrMessageEvent):
        ...
        if res and res.chain:
            for item in res.chain:
                if isinstance(item, MsgImage) and not getattr(item, 'is_chuan_rendered', False):
                    return # 发现 P 站等插件的图，主动跳过渲染
        if not self._cfg_bool('enable_render', True):
            return
        ...
        # 提取表情标签
        emotion_tag = event.get_extra('extracted_emotion_tag')
        if not emotion_tag:
            full_text = ''
            for item in res.chain:
                if isinstance(item, Comp.Plain) or hasattr(item, 'text'):
                    full_text += getattr(item, 'text', '') or ''
            if full_text:
                emotion_tag, _ = self._emotion_from_text(full_text)
        ...
        if not emotion_tag:
            ...
            for item in res.chain:
                if hasattr(item, 'text'):
                    t = getattr(item, 'text', '')
                    if t:
                        _, ct = self._emotion_from_text(t)
                        item.text = ct
            return
        
        # 确定需要渲染的模式
        mode = self.cfg().get('render_mode', 'full')
        logger.info('[传话筒] 为会话 %s 渲染表情: %s (模式: %s)', session_id, emotion_tag, mode)
        ...
        try:
            ...
            # 使用支持分割长文本的函数
            components = await self._render_split_text(cleaned_text, emotion_tag, session_id)
            if components:
                if mode == 'full':
                    res.chain.clear()
                    res.chain.extend(components)
                else:
                    # split 模式: 只把文本换成图，保留其他（如有）
                    new_chain = []
                    new_chain.extend(components)
                    for item in res.chain:
                        if not isinstance(item, PLAIN_COMPONENT_TYPES):
                            new_chain.append(item)
                    res.chain.clear()
                    res.chain.extend(new_chain)
        except Exception as e:
            logger.error('[传话筒] 渲染过程出错: %s', e)
        return
    ...
    @filter.command("传话筒保留文本")
    async def command_keep_original_text(self, event: AstrMessageEvent, status: str):
        if not self._check_control_permission(event):
            yield event.plain_result("你没有权限使用此指令。")
            event.stop_event()
            return
        enable = status.strip() in ["开启", "on", "true", "1"]
        session_id = event.unified_msg_origin
        if enable:
            self._keep_text_sessions.add(session_id)
        else:
            if session_id in self._keep_text_sessions:
                self._keep_text_sessions.remove(session_id)
        self._save_keep_text_sessions()
        msg = f"已为当前会话设置传话筒保留原文本：{'开启' if enable else '关闭'}"
